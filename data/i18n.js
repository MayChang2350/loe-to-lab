...
const UI = {
  ...
  pw: {
    ...
    jurForm:   { en: 'Form / dossier', zh: '申請表件' },
    jurTimeline: { en: 'Typical timeline', zh: '典型時程' },
    jurOdds:   { en: 'Chances of approval', zh: '核准機率' },
    jurContact: { en: 'Who to contact', zh: '聯絡窗口' },
    jurFdaOnlyLbl: { en: 'Scope note', zh: '範圍說明' },
    jurFdaOnly: {
      en: 'The clock and the economics below model FDA statutory exclusivities and FY2026 user fees only. With {jur} selected above, treat these as an FDA-equivalent reference point, not as {jur} figures — check the jurisdiction panel\'s links for the real numbers.',
      zh: '下方的時鐘與經濟模型僅採用 FDA 法定專屬權與 2026 會計年度使用者費用。目前選取的是 {jur}，請將以下內容視為 FDA 對應參考點，而非 {jur} 的實際數字——正確數字請查閱上方主管機關面板的連結。'
    },
    jurEquivTitle: { en: 'Closest FDA-equivalent pathway', zh: '最接近的 FDA 對應路徑' },
    jurGenericLbl: { en: 'Generic / hybrid route', zh: '學名藥／混合型途徑' },
    jurBiosimLbl: { en: 'Biosimilar route', zh: '生物相似藥途徑' },
  ...
  /* --- Process lab (tab shell around the fluid bed, six unit operations
     and five analytical instruments) --- */
  lab: {
    title: { en: 'Process lab', zh: '製程實驗室' },
    lead: { en: 'The machines that make the batch, and the instruments that measure it — twelve stations from a real CDMO process and QC lab. Move a control and watch the live schematic, the numbers and the plain-language verdict all respond together.', zh: '製造批次的機台，以及量測成品的儀器——十二個站台，取材自真實 CDMO 製程與品管實驗室。移動任一控制項，即時示意圖、數字與白話判讀會一起反應。' },
    ...
    reflection: {
      ...
      body: {
        en: [
          ...
          'None of this would have happened without the people who made room for it. Mink connected me to this opportunity in the first place, through CFO Alice Wang — a conversation I almost didn\'t think to have, and the one that put me in this building. Dr. Qiu gave me industry guidance and support throughout the summer. And Nick taught me nearly everything hands-on: wet lab technique, how a real manufacturing floor actually runs, and a deep dive into reverse engineering and fluid-bed thermodynamics. More than he probably realizes, he\'s responsible for most of what made this website possible — almost every control on this site exists because he explained what it actually does first.',
          ...
          'Thank you — to Mink, to Alice, to Dr. Qiu, to Nick, to my fellow intern Charlie, and to everyone else who made room for someone still learning the difference between a d50 and a D50.'
        ],
        zh: [
          ...
          '這一切都少不了願意為此騰出空間的人。Mink 最早把這個機會介紹給我，透過 CFO Alice Wang 牽線——那是一次我差點沒想過要去進行的談話，卻是把我帶進這棟大樓的關鍵一步。仇博在這整個夏天裡，給了我產業上的指引與支持。而 Nick 幾乎手把手教了我所有實作的東西：濕實驗室技巧、真實產線是如何運作的，還有深入的逆向工程與流體床熱力學。大概比他自己意識到的還多，這個網站之所以能做出來，大半都要歸功於他——網站上幾乎每一個控制項的存在，都是因為他先解釋了它實際上在做什麼。',
          ...
          '謝謝——謝謝 Mink、謝謝 Alice、謝謝仇博、謝謝 Nick、謝謝我的實習夥伴 Charlie，也謝謝每一位為一個還在搞清楚 d50 和 D50 差別的人，騰出空間的人。'
        ]
      },
