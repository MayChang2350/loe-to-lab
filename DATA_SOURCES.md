...
- **The other six process models.** Lumped and empirical. Each reproduces the direction and rough magnitude of every response and puts the failure modes in the right place, but none is calibrated to a specific product or machine. Where a calibration constant was chosen to make a threshold land somewhere sensible — the surfactant coverage scale in the emulsion model, the yield pressure in the compaction model, the mixing rate constant in the blender — the choice is commented in `data/unitops.js` at the line where it is used.
- **Capsule diagram.** Drawn from the FDA-approved label text (white to off-white opaque hard gelatin capsule, grey "FL 145" imprint, linaclotide-coated beads) rather than traced from a photograph. Bead count, packing and layer thickness are schematic. The official photograph is linked from the figure.
